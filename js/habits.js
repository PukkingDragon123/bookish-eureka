...
const Habits = (() => {
  ...
  const DEFS = [
    { id: 'wake', icon: 'sunrise', name: 'Morning Check-in', kind: 'instant', perDay: 1,
      desc: 'Greet the day. Grants the Dawn Blessing: +15% gold & XP for 4h.',
      mana: 20, xp: 30, ess: 0,
      goal: 'mind' },
    { id: 'water', icon: 'water', name: 'Drink Water', kind: 'instant', perDay: 8,
      desc: 'Each glass channels 12 mana toward your next summon.',
      mana: 12, xp: 6, ess: 0,
      goal: 'mind' },
    { id: 'meal', icon: 'meal', name: 'Eat a Healthy Meal', kind: 'meal', perDay: 3,
      desc: 'Log what you ate (and it feeds the calorie ring below).',
      mana: 25, xp: 16, ess: 1,
      goal: 'nutrition' },
    { id: 'cook', icon: 'cook', name: 'Cook Something', kind: 'instant', perDay: 1,
      desc: 'Home-cooked = beast-approved. Bonus essence for the craft.',
      mana: 18, xp: 22, ess: 2,
      goal: 'nutrition' },
    { id: 'walk', icon: 'walk', name: 'Take a Walk', kind: 'timer', perDay: 2, mins: [10, 15, 20, 30],
      desc: 'A real-time walk timer. Finish it for mana & essence.',
      manaPerMin: 1.2, xpPerMin: 2, essPer10: 1,
      goal: 'fitness' },
    { id: 'exercise', icon: 'muscle', name: 'Exercise Session', kind: 'timer', perDay: 2, mins: [10, 15, 20, 30, 45, 60],
      ...
      goal: 'fitness' },
    { id: 'create', icon: 'palette', name: 'Create / Practice', kind: 'timer', perDay: 2, mins: [15, 25, 45],
  ...
  function completeInstant(def, btnEl) {
    ...
    if (def.id === 'wake') {
      ...
      toast('Dawn Blessing active: +15% gold and XP for 4h', 'gold');
    }
    ...
    if (S.mealsDay !== today) {
      ...
      if (S.mealsDay && S.meals.length > 0) {
        ...
        if (total > 0 && total <= S.kcalTarget && S.kcalBonusDay !== S.mealsDay) {
          ...
          grantGems(8);
          grantEssence(6);
          toast('You finished yesterday within your calorie target! +8 gems, +6 essence', 'gold');
        }
      }
      S.meals = [];
      S.mealsDay = today;
  ...
  function celebrate(fromEl, mana, xp, ess) {
    ...
    if (fromEl) {
      ...
      if (mana) mkFloat(`+${mana} MANA`, 'mana-f', 0);
      if (xp) setTimeout(() => mkFloat(`+${xp} XP`, 'xp-f', 8), 140);
      if (ess) setTimeout(() => mkFloat(`+${ess} ESS`, 'heal', 16), 280);
    }
  }

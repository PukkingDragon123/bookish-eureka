...
const UI = (() => {
  ...
  function renderEnemyHp(enemy) {
    $('#enemy-hpbar i').style.width = Math.max(0, 100 * enemy.hp / enemy.hpMax) + '%';
  ...
  function renderParty() {
    ...
    for (const cid of S.party) {
      ...
      f.innerHTML = `<img src="${sprite(cid)}" alt="${C_BY_ID[cid].name}"><span class="flvl">Lv.${st.lvl}</span>`;
  ...
  let lungeIdx = 0;
  function showHit(dmg, crit) {
    const scene = $('#scene');
    const eh = $('#enemy-holder');
    if (!eh) return;
    const r = eh.getBoundingClientRect(), sr = scene.getBoundingClientRect();
    floatText(fmt(dmg), r.left - sr.left + r.width / 2, r.top - sr.top + 40, crit ? 'crit' : '');
    const img = $('#enemy-img');
  ...
  function showKillRewards(gold, mult) {
    ...
    const eh = $('#enemy-holder');
    const r = eh.getBoundingClientRect(), sr = scene.getBoundingClientRect();
    floatText(`+${fmt(gold)} 🪙${mult > 1 ? ' x' + mult : ''}`, r.left - sr.left + r.width / 2, r.top - sr.top + 90, 'reward');
    const img = $('#enemy-img');
  ...
  function renderDexFilters() {
    ...
    for (const o of opts) {
      const b = el('button', 'dfilter' + (dexFilter === o ? ' on' : ''),
        o === 'all' ? 'All' : o === 'owned' ? 'Owned' : `${TYPE_ICONS[o]} ${o}`);
  ...
  function showCreature(cid) {
    ...
    box.innerHTML = `
      ...
      </div>` : `<p class="subtle" style="margin-top:8px">Not yet bonded — summon or evolve to recruit.</p>`}
      ${evoHtml}
    ...
    if (owned) {
      ...
      const lu = el('button', 'big-btn gold-btn small');
      lu.innerHTML = `<b>Level Up</b><span>🪙 ${fmt(cost)}</span>`;
      ...
      if (req) {
        const ev = el('button', 'big-btn small');
        ev.innerHTML = `<b>Evolve</b><span>Lv.${req.lvlReq} + ${req.essReq}✨</span>`;
        const can = S.beasts[cid].level >= req.lvlReq && S.player.essence >= req.essReq;
        ev.disabled = !can;
      ...
      const pt = el('button', 'big-btn small ' + (inParty ? '' : 'green-btn'));
      pt.innerHTML = inParty ? '<b>Rest</b><span>leave party</span>' : '<b>Fight!</b><span>join party</span>';
      pt.onclick = () => {
        if (inParty) {
  ...
  function doEvolve(cid) {
    ...
    const ok = el('button', 'big-btn', '<b>Magnificent!</b>');
  ...
  function renderSummon() {
    ...
    for (const r of RELICS) {
      ...
      d.innerHTML = `<span class="ric">${n ? r.icon : '❔'}</span><b>${n ? r.name : '???'}</b><span class="rbonus">${n ? r.desc + (n > 1 ? ` (x${n})` : '') : 'undiscovered'}</span>`;
      shelf.appendChild(d);
    }
  }
  ...
  function showSummonReveal(c, isNew, dupBonus) {
    ...
    const back = openModal(box);
    const again = el('button', 'big-btn mana-btn small');
    again.innerHTML = `<b>Summon again</b><span>🔮 ${Summon.MANA_COST}</span>`;
  ...
  function showRelicReveal(r) {
    ...
    box.innerHTML = `
      ...
      <div class="burst"><div class="rays"></div><div style="font-size:90px;z-index:1">${r.icon}</div></div>
      <h2>${r.name}</h2>
  ...
  function ritualCard(def, isCustom) {
    ...
    const prog = max > 1
      ...
      : '';
    let rewardTxt;
    if (def.kind === 'timer') rewardTxt = `⏱ real timer · ~${Math.round((def.manaPerMin || 1) * 15)}🔮 / 15min${def.boost ? ' · ⚡x3 idle boost' : ''}`;
    else rewardTxt = `+${def.mana}🔮 +${def.xp}XP${def.ess ? ` +${def.ess}✨` : ''}`;
    card.innerHTML = `
    ...
    if (def.kind === 'timer') {
      ...
      btn = el('button', 'rbtn timer', running ? '⏱ ' + fmtTime(Habits.timerRemaining() / 1000) : 'Start');
      btn.disabled = complete && !running;
      btn.onclick = () => running ? showTimerModal() : showTimerStart(def);
    } else if (def.kind === 'meal') {
      btn = el('button', 'rbtn', complete ? '✓ Done' : `Log (${done}/${max})`);
      btn.disabled = complete;
      btn.onclick = e => showMealModal(true);
    } else {
      btn = el('button', 'rbtn', complete ? '✓ Done' : max > 1 ? `+1 (${done}/${max})` : 'Done!');
      btn.disabled = complete;
  ...
  function showTimerStart(def) {
    ...
    box.innerHTML = `<h3>${def.icon} ${def.name}</h3>
    ...
    const start = el('button', 'big-btn gem-btn', '<b>Begin ritual</b>');
    start.onclick = () => {
  ...
  function showTimerModal() {
    ...
    const box = el('div', 'timer-wrap');
    box.innerHTML = `<h3>${def.icon} ${def.name} — ${S.exTimer.mins} min</h3>
    ...
    const give = el('button', 'ghost-btn', 'Abandon');
    give.style.margin = '0';
  ...
  function showMealModal(healthyDefault) {
    const box = el('div');
    box.innerHTML = `<h3>🍽️ Log Food</h3>
      ...
      <div class="mrow"></div>`;
    const row = box.querySelector('.mrow');
    const ok = el('button', 'big-btn green-btn', '<b>Log it</b>');
    ok.onclick = e => {
  ...
  function showKcalTargetModal() {
    ...
    box.innerHTML = `<h3>🎯 Daily Calorie Target</h3>
      ...
      <div class="mrow"></div>`;
    const ok = el('button', 'big-btn', '<b>Set target</b>');
    ok.onclick = () => {
  ...
  function showAddCustomModal() {
    const box = el('div');
    const icons = ['⭐', '📚', '🧹', '🛏️', '🌤️', '🎸', '🧠', '💌', '🌻', '🪥'];
    box.innerHTML = `<h3>＋ Create a Ritual</h3>
      <label>Name it</label>
    ...
    icons.forEach(ic => {
      const b = el('button', 'preset' + (ic === sel ? ' on' : ''), ic);
      b.onclick = () => { sel = ic; $$('.preset', irow).forEach(x => x.classList.remove('on')); b.classList.add('on'); };
      irow.appendChild(b);
    });
    const ok = el('button', 'big-btn green-btn', '<b>Add ritual</b>');
    ok.onclick = () => {
  ...
  function renderQuests() {
    ...
    for (const q of S.quests.list) {
      ...
      const rewardBits = [];
      if (q.reward.gems) rewardBits.push(q.reward.gems + '💎');
      if (q.reward.mana) rewardBits.push(q.reward.mana + '🔮');
      if (q.reward.ess) rewardBits.push(q.reward.ess + '✨');
      if (q.reward.gold) rewardBits.push('🪙');
      card.innerHTML = `
        <div class="qic">${q.icon}</div>
        <div class="qmain">
          <div class="qname">${q.name}</div>
          <div class="qbar"><i style="width:${100 * q.progress / q.target}%"></i></div>
          <div class="qprog-txt">${q.progress}/${q.target} · ${rewardBits.join(' ')}</div>
        </div>`;
      const btn = el('button', 'qclaim', q.claimed ? 'Claimed ✓' : done ? 'Claim!' : 'Claim');
    ...
    j.innerHTML = '';
    const stats = [
      ['🔥 Current streak', S.streak.count + ' day' + (S.streak.count === 1 ? '' : 's')],
      ['⚔️ Beasts defeated', fmt(S.kills)],
      ['☠️ Bosses slain', fmt(S.bossKills)],
      ['🐾 Companions bonded', Object.keys(S.beasts).length],
      ['🔮 Summons performed', S.summons.total],
      ['📅 Journey started', new Date(S.created).toLocaleDateString()],
    ];
    for (const [k, v] of stats) j.appendChild(el('div', 'jstat', `<span>${k}</span><b>${v}</b>`));
  }

  /* ================= settings ================= */
  function showSettings() {
    const box = el('div');
    box.innerHTML = `<h3>⚙️ Settings</h3><div class="mrow" style="flex-direction:column;align-items:stretch"></div>`;
    const row = box.querySelector('.mrow');
    const snd = el('button', 'ghost-btn', (S.settings.sound ? '🔊 Sound: ON' : '🔇 Sound: OFF'));
    snd.style.margin = '0';
    snd.onclick = () => {
    ...
    row.appendChild(snd);
    const reset = el('button', 'ghost-btn', '🗑️ Reset all progress');
  ...
  function showOnboarding() {
    ...
    for (const [key, g] of Object.entries(GOALS)) {
      ...
      c.innerHTML = `<div class="gic">${g.icon}</div><div class="gname">${g.name}</div><div class="gdesc">${g.desc}</div>`;
      c.onclick = () => {
    ...
    const next = el('button', 'big-btn', '<b>Choose my companion ➤</b>');
    next.disabled = true;
    next.onclick = () => {
      S.goals = [...chosen];
      closeModal(back);
  ...
  function showStarterPick() {
    ...
    box.innerHTML = `
      ...
      <p>${GOALS[goal].icon} Matched to your <b>${GOALS[goal].name}</b> path.<br>It will grow and evolve as <b>you</b> do.</p>
    ...
    const go = el('button', 'big-btn green-btn', '<b>Begin the journey!</b>');
    go.disabled = true;
    go.onclick = () => {
      S.starterCid = sel;
      ownBeast(sel);
